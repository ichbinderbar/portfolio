...
SOC 2 replaced the SAS 70 standard, which was originally designed for evaluating financial controls at service organizations. As cloud computing grew, the industry needed a framework that went beyond financial reporting to address security, availability, and data handling. In 2010, the AICPA introduced three new report types — SOC 1 (financial controls), SOC 2 (Trust Services Criteria), and SOC 3 (public summary) — under SSAE 16. In 2017, SSAE 18 became the governing standard, and the AICPA published the Trust Services Criteria (TSP Section 100) that currently defines SOC 2 audits. The criteria were updated with revised Points of Focus in 2022 to address evolving threats, though the core criteria themselves remain unchanged.
...
Security alone contains 32 individual criteria. For many companies — especially those early in their compliance journey — starting with Security only is the right move.
...
SOC 2 reports are typically shared under NDA. Most companies share Section 1 (auditor's opinion) and Section 4 (test results) with prospects during vendor due diligence. If you want to publicly demonstrate your security posture, get a SOC 3 report — it's a summary designed for public distribution.
